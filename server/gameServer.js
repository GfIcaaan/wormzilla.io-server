...
const { WorldState, Player, calcSegmentCount, packPosToId } = require('./worldState');
...
function buildUpdatePacket(me, topPlayers, dt) {
  ...
  // Section 2: Eaten food (id only)
  // NOTE: must send the SAME packed-position id the client received when this
  // food was spawned (Section 1), since the client looks the food up in its
  // dictionary by that exact id (this.h.Wb[x.G]) to remove/animate it. Sending
  // the raw internal food.id here would never match what the client has stored.
  w.writeVarInt(world.eatenFood.length);
  for (const f of world.eatenFood) {
    w.writeUint32(packPosToId(f.x, f.y));
  }

  // Section 3: Collected abilities (ability id + collecting player's id)
  // Same reasoning as Section 2: the id must be the packed-position id the
  // client already has stored for this ability orb, not the raw internal id.
  w.writeVarInt(world.collectedAbilities.length);
  for (const c of world.collectedAbilities) {
    w.writeUint32(packPosToId(c.x, c.y));
    w.writeInt16(c.playerId);
  }
  ...
  // Section 6: Worm position updates (send all visible players)
  const livePlayers = Array.from(world.players.values()).filter(p => !p.dead && !p.spectating);
